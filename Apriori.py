...
DataFile = open('data_permits_5000.csv', 'r')
minsup = 0.03
f2 = "Rules.txt"
f1 = "FItems.txt"
minconf = 0.39
...
def L1():
    ...
    #Thresholding
    count2 = {k: v for k, v in count.items() if v >= minsup*total_num}
        ...
        for i in range(1, length):
            ...
            for left in lefts:
                ...
                if conf >= minconf:
                    ...
                    right = list(itemset[:])
                    conf=round(conf,2)
                    lift = union_support/leftcount/right
                    lift=round(lift,2)

                    for e in left:
                        right.remove(e)
                    fo.write(str(left) + ' (' + str(leftcount) + ')' + ' -> ' + str(right) + ' (' + str(fitems[''.join(right)]) + ')' + ' [' + str(conf) + ']'+'[' +str(lift)+']'+ '\n')
